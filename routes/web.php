...
Route::get('/bukti-pengajuan/{nomor}/pdf',[PendaftarController::class, 'downloadBuktiPdf']);
Route::get('/validasi/{nomor}', [PendaftarController::class, 'validasi']);
...
Route::middleware(['auth', 'reviewer'])->prefix('admin')->group(function () {
    Route::get('/dashboard', function () {
        ...
        $menungguReview = Pendaftar::where('status_seleksi', 'Menunggu Review')->count();
        $sedangDireview = Pendaftar::where('status_seleksi', 'Sedang Direview')->count();
        $perluRevisi = Pendaftar::where('status_seleksi', 'Perlu Revisi')->count();
        $diterima = Pendaftar::where('status_seleksi', 'Diterima')->count();
        $dipublikasikan = Pendaftar::where('status_seleksi', 'Dipublikasikan')->count();
    ...
    Route::get('/pendaftar', [PendaftarController::class, 'adminIndex']);
    Route::get('/pendaftar/{id}', [PendaftarController::class, 'showAdmin']);
    Route::post('/pendaftar/{id}/verifikasi', [PendaftarController::class, 'verifikasi']);
});

Route::middleware(['auth', 'admin'])->prefix('admin')->group(function () {
    Route::resource('/formasi', FormasiController::class);

    Route::get('/berkas', [BerkasController::class, 'adminIndex']);
    Route::post('/berkas/{id}/verifikasi', [BerkasController::class, 'verifikasi']);

    Route::get('/hasil-seleksi', [HasilSeleksiController::class, 'adminIndex']);
